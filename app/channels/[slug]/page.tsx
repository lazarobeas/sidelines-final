...
export default function ChannelPage() {
  ...
  useEffect(() => {
    ...
    async function fetchChannelData() {
      ...
      try {
        ...
        if (channelData) {
          // Fetch highlights for all game statuses
          ...
          // Only fetch messages if the game is not completed
          if (channelData.game_status !== "COMPLETED") {
            ...
            setMessages([]); // Ensure messages array is empty for completed games
          }
        }
        setActiveUsers(Math.floor(Math.random() * 120) + 5); // Mock active users
      } catch (error) {
  ...
  useEffect(() => {
    if (!channel?.id || !slug) return;

    const commonFilter = `channel_id=eq.${channel.id}`;
    let messagesSubscriptionInstance: ReturnType<typeof supabase.channel> | null = null;

    // Only subscribe to messages if the game is not completed
    if (channel.game_status !== "COMPLETED") {
      messagesSubscriptionInstance = supabase
        .channel(`messages_channel_${channel.id}`)
    ...
    const highlightsSubscription = supabase
        .channel(`highlights_channel_${channel.id}`)
    ...
    return () => {
      if (messagesSubscriptionInstance) {
        supabase.removeChannel(messagesSubscriptionInstance);
      }
      supabase.removeChannel(highlightsSubscription);
    };
  }, [channel?.id, channel?.game_status, slug, supabase]); // Added channel.game_status to dependencies
  ...
  // Render Game Summary View for COMPLETED games
  if (channel.game_status === "COMPLETED") {
    return (
      <div className="h-screen w-screen bg-[#232341] flex flex-col overflow-hidden">
        <header className="bg-[#1a1a33]/80 backdrop-blur-sm p-3 md:p-4 shadow-md flex-shrink-0 border-b border-blue-900/30 z-10">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <div className="flex items-center min-w-0">
              <Link href="/feed" className="mr-2 md:mr-3 text-gray-300 hover:text-white transition-colors">
                <ChevronLeft className="h-5 w-5 md:h-6 md:w-6" />
              </Link>
              <div className="flex items-center overflow-hidden mr-2">
                <div className="w-6 h-6 md:w-7 md:h-7 rounded-full mr-1.5 md:mr-2 flex-shrink-0 flex items-center justify-center p-0.5 border border-white/10" style={{ backgroundColor: `${channel.home_team.primary_color}4D` }} >
                  <img src={channel.home_team.logo_url || "/team-placeholder.png"} alt={channel.home_team.name} className="w-full h-full object-contain"/>
                </div>
                <div className="text-xs sm:text-sm md:text-base text-white font-semibold truncate" title={`${channel.home_team.name} vs ${channel.away_team.name}`}>
                  <span className="hidden sm:inline">{channel.home_team.name}</span>
                  <span className="sm:hidden">{channel.home_team.name.substring(0,3).toUpperCase()}</span>
                  <span className="mx-1">vs</span>
                  <span className="hidden sm:inline">{channel.away_team.name}</span>
                  <span className="sm:hidden">{channel.away_team.name.substring(0,3).toUpperCase()}</span>
                </div>
                <div className="w-6 h-6 md:w-7 md:h-7 rounded-full ml-1.5 md:ml-2 flex-shrink-0 flex items-center justify-center p-0.5 border border-white/10" style={{ backgroundColor: `${channel.away_team.primary_color}4D` }} >
                  <img src={channel.away_team.logo_url || "/team-placeholder.png"} alt={channel.away_team.name} className="w-full h-full object-contain"/>
                </div>
              </div>
            </div>
            <div className="flex items-center flex-shrink-0">
              <Badge className={`text-[10px] sm:text-xs px-2 py-0.5 md:px-2.5 md:py-1 mr-2 md:mr-3 rounded-full border border-transparent bg-gray-500/90 text-white border-gray-400/50`}>
                {channel.game_status}
              </Badge>
              <div className="text-white font-bold text-xs sm:text-sm md:text-base mr-2 md:mr-3 hidden xs:block">
                {channel.score_home}-{channel.score_away}
              </div>
              <div className="text-gray-300 flex items-center text-[10px] sm:text-xs md:text-sm">
                <Users className="h-3 w-3 md:h-4 md:w-4 mr-1" />
                <span>{activeUsers}</span>
                <span className="hidden xs:inline ml-0.5 md:ml-1">fans</span>
              </div>
            </div>
          </div>
        </header>

        <div className="flex-grow flex flex-col min-h-0 p-2 md:p-3 gap-2 md:gap-3 overflow-y-auto custom-scrollbar">
            <div className="bg-[#1c1c3a] p-3 md:p-4 rounded-lg shadow-xl border border-blue-900/30 mb-3 md:mb-4 flex-shrink-0">
              <h2 className="text-white text-lg sm:text-xl font-bold mb-3 text-center border-b border-blue-900/50 pb-2">
                Final Score
              </h2>
              <div className="flex justify-around items-center text-white py-2">
                <div className="text-center w-2/5">
                  {channel.home_team.logo_url ?
                    <img src={channel.home_team.logo_url} alt={channel.home_team.name} className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-1.5 object-contain"/>
                    : <div className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-1.5 rounded-full bg-gray-700 flex items-center justify-center text-xl font-bold" style={{color: channel.home_team.primary_color || '#FFFFFF'}}>{channel.home_team.name.substring(0,1)}</div>
                  }
                  <span className="text-sm md:text-base font-semibold block truncate">{channel.home_team.name}</span>
                  <p className="text-2xl md:text-3xl font-bold mt-1">{channel.score_home}</p>
                </div>
                <div className="text-2xl md:text-4xl font-light text-gray-500 px-2">vs</div>
                <div className="text-center w-2/5">
                   {channel.away_team.logo_url ?
                    <img src={channel.away_team.logo_url} alt={channel.away_team.name} className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-1.5 object-contain"/>
                    : <div className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-1.5 rounded-full bg-gray-700 flex items-center justify-center text-xl font-bold" style={{color: channel.away_team.primary_color || '#FFFFFF'}}>{channel.away_team.name.substring(0,1)}</div>
                  }
                  <span className="text-sm md:text-base font-semibold block truncate">{channel.away_team.name}</span>
                  <p className="text-2xl md:text-3xl font-bold mt-1">{channel.score_away}</p>
                </div>
              </div>
              <p className="text-center text-gray-400 text-xs sm:text-sm mt-3 pt-2 border-t border-blue-900/30">
                Played at {channel.venue} &middot; {new Date(channel.game_date).toLocaleDateString([], { year: 'numeric', month: 'long', day: 'numeric' })}
              </p>
            </div>
  ...
  // Existing return for LIVE or UPCOMING games
  return (
      <div className="h-screen w-screen bg-[#232341] flex flex-col overflow-hidden">
